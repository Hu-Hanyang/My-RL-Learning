...
class Agent(object):
    ...
    def learning(self, lambda_=0.9, epsilon=None, decaying_epsilon=True, gamma=0.9,
        ...
        total_time, episode_reward, num_episode = 0, 0, 0  # 1 episode
        total_times, episode_rewards, num_episodes = [], [], []  # all episodes
        for _ in tqdm(range(max_episode_num)):
            if epsilon is None:
                epsilon = 1e-10
            elif decaying_epsilon:
                epsilon = 1.0 / (1 + num_episode)
            time_in_episode, episode_reward = self.learning_method(lambda_=lambda_, gamma=gamma, alpha=alpha, epsilon=epsilon,
                                                                   display=display)
        ...
        return total_times, episode_rewards, num_episodes  # 3 lists which contain all messages of each episode

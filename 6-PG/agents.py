...
from network import NetApproximator
import torch
...
class DDQNAgent(Agent):
    def __init__(self, env=None, capacity=20000, hidden_dim=32, batch_size=128, epochs=2):
        if env is None:
            raise "Agent should have an environment!"
        super(DDQNAgent, self).__init__(env, capacity)
        self.input_dim = env.observation_space.shape[0]
        self.output_dim = env.action_space.n  # 这个n是什么属性？
        self.hidden_dim = hidden_dim
    ...
    def policy(self, A, s, Q=None, epsilon=None):
        Q_s = self.behavior_Q(s)  # 使用了__call__方法？？
        rand_value = random()  # 生成0到1之间到随机数
        if epsilon is not None and rand_value < epsilon:
            return self.env.action_space.sample()
        else:
            return int(np.argmax(Q_s))
    ...
    def learning_method(self, lambda_=0.9, gamma=0.9, alpha=0.1, epsilon=13-5, display=False):
        ...
        while not is_done:
            ...
            if display:
                self.env.render()
            if self.total_trans > self.batch_size:   # memory中的状态转换足够度多
                loss += self._learn_from_memory(gamma, alpha)  # 从memory中学习
            time_in_episode += 1
